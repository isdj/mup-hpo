...
from mup import set_base_shapes, load_base_shapes, make_base_shapes
...
def vit_base_patch8_384(pretrained=False, **kwargs):
    """ViT-Base segmenter (ViT-B/16) from original paper (https://arxiv.org/abs/2010.11929).
...
def create_segmenter(model_cfg):
    ...
    if model_cfg['mup']:
        ...
        decoder = create_decoder(encoder, decoder_cfg)
        delta_model = Segmenter(encoder, decoder, n_cls=model_cfg["n_cls"])

        set_base_shapes(model, make_base_shapes(model, delta_model) if model_cfg['mup'] else None)
