...
from mup import MuReadout, trunc_normal_ as mup_trunc_normal_
from models.segmenter.blocks import Attention
...
def init_weights(m,readout_zero_init: bool = False,
            query_zero_init: bool = False):
    if isinstance(m, nn.Linear):
        if isinstance(m, MuReadout) and readout_zero_init:
            m.weight.data.zero_()
        elif hasattr(m.weight, 'infshape'):
            mup_trunc_normal_(m.weight, std=0.02)
        else:
...
def checkpoint_filter_fn(state_dict, model):
    ...
    out_dict = {}
    if "segmenter" in state_dict:
        # For deit models
        state_dict = state_dict["segmenter"]
    num_extra_tokens = 1 + ("dist_token" in state_dict.keys())
    patch_size = model.patch_size
    image_size = model.patch_embed.image_size
    for k, v in state_dict.items():
        if k == "pos_embed" and v.shape != model.pos_embed.shape:
            # To resize pos embedding when using segmenter at different size from pretrained weights
            v = resize_pos_embed(
                v,
                None,
